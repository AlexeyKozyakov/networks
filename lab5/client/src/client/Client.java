...
public class Client {
    ...
    private void openChat(LoginResponse loginResponse) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            updating.start();
            new Thread(messagesTask).start();
        });
    }

    private void getPreviousMessages() {
        ResponseResult messagesResult = doGetAuthorizedRequest("/messages?offset=" + 0 + "&count=" + -1);
    ...
    private Runnable messagesTask = () -> {
        ClientManager clientManager = ClientManager.createClient();
        try {
            int webSocketPort = Integer.valueOf(port) + 1;
            Session session = clientManager.connectToServer(new MessagesEndpoint(this),
                    new URI("ws://" + url.substring(url.indexOf("/") + 2) + ":" + webSocketPort + "/ws/messages"));
             session.getBasicRemote().sendText(token.toString());
